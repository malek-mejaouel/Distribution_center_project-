...
void MainWindow::handleLogin() {
    ...
    User user;
    //switch case (user.login(username,password)="Employee" {
        // Dialog *Dash = neww Dialog(this);
        // case1 : ui->tab1->EmployeeDashboard(true); }
        // case2 : ui->tab1->InstitutDashboard(true); }
        // case3 : ui->tab1->BoxDashboard(true); }
        // case4 : ui->tab1->EquipDashboard(true); }
        // case5 : ui->tab1->LivreurDashboard(true); }
    if (user.login(username, password)) {
        failedLoginAttempts = 0; // Reset the failed attempts counter on successful login
        QMessageBox::information(this, "Login Successful", "Welcome, " + username + "!");
        Dialog *dash = new Dialog(this);
        dash->setModal(true);
        connect(dash, &Dialog::signOutRequested, this, [=]() {
            this->show();  // Show main window again on sign out
        });
        dash->show();
        ...
        if (failedLoginAttempts >= 3) {
            ui->log->setEnabled(false);  // Disable the login button
            lockoutTimer->start(10000);  // Lockout for 30 seconds (you can adjust this)
            QMessageBox::critical(this, "Blocked", "Too many failed attempts. You are temporarily blocked.");
        }else {
            QMessageBox::critical(this, "Login Failed", "Invalid username or password. Attempt " + QString::number(failedLoginAttempts) + " of 3.");
        }
    }
}
